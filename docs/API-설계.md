...
|---|---|---|---|
| POST | `/api/auth/signup` | `email, password, name` | 이메일 UNIQUE. 소프트웨어학과 1개 학과만 다루므로 학과 선택 UI 없음 |
...
| Response status | data |
|---|---|
| 200 | `{ "status":200, "code":"ME_SUCCESS", "data":{ "id":1, "name":"홍길동", "department":"소프트웨어학과" } }` |
...
| Response status | data |
|---|---|
| 200 | `{ "status":200, "code":"CHAT_MESSAGE_SUCCESS", "data":{ "role":"assistant", "content":"전공필수 3과목이 더 필요해요.", "citedChunks":[ { "chunkId":12, "documentTitle":"소프트웨어학과 이수규정", "excerpt":"..." } ] } }` |
...
> 이수규정 자체가 아직 미수집 상태입니다 — `curriculum_requirements`/`curriculum_required_courses` 시드는 소프트웨어학과 이수규정 원문 확보 후 채웁니다.

---

## 5. 남은 확인 사항
- 카탈로그 51개 과목의 요일/교시 데이터 재수집 필요 (3.1 참고)
- 소프트웨어학과 이수규정 원문 확보 필요 (4.1 참고)
