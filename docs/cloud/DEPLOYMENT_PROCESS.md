...
본 문서는 **Vercel(프론트엔드 및 서빙)과 Aiven(클라우드 MySQL DB)을 연동하고 GitHub 자동 배포 파이프라인(CI/CD)을 구축**하여, 개발팀 소스코드가 `develop` 및 `main` 브랜치에 머지되는 즉시 **무상 HTTPS 적용, PR 단위 사전 테스트(Preview), 인프라 관리 공수 제로(0)화가 보장되는 고가용성 무중단 배포 환경**을 수립하는 표준 구축 과정과 협업 지침을 기록한 기술 문서입니다.

---

## 👤 2. 기본 정보

* **작성자:** 배포 담당자 (DevOps Practitioner)
* **대상 서비스:** Web Client (Vite/React), Express Backend, MySQL Database Monorepo
* **주요 인프라 스택:** 
  ...
  * **CI/CD & Version Control:** GitHub Repository (`develop` & `main` 브랜치 전략, Vercel GitHub Bot)
...
[Vercel Cloud Platform]
   ...
   └── [Serverless Functions / API Layer]
           │
           └── (3) Encrypted MySQL Connection (SSL/TLS, Port: <DB_PORT>)
                   ▼
      [Aiven Cloud Platform (SaaS)]
            └── [MySQL 8.0 Database (<DB_NAME>)] ── (Volume: Cloud Storage)
```
...
### Phase 1. 클라우드 MySQL DB 개설 및 스키마 주입
* **작업 내용**: 24시간 무상 가동되는 클라우드 MySQL DB 인스턴스를 확보하고 애플리케이션 스키마 구축.
* **진행 과정**:
  1. **독립된 Aiven 인스턴스 확보**: Aiven Cloud에서 무료 MySQL 8.0 인스턴스를 생성하고 5가지 접속 자격 증명(Host, Port, User, Password, Database Name) 확보.
  2. **마이그레이션 스크립트 실행을 통한 스키마 주입**:
     - Aiven for MySQL 콘솔의 환경 특성에 맞춰, 로컬 Node.js + `mysql2` 기반의 1회성 마이그레이션 스크립트를 작성하여 안전하게 실행.
     - `DB_HOST=... DB_PASSWORD=... node scratch/init-aiven-db.js` 형태의 명령을 통해 지정된 클라우드 DB에 `db/schema.sql` 전체 DDL을 전송하고 테이블 생성을 자동 검증.
* **결과**: 기존 `db/schema.sql`과 백엔드 `mysql2` 커넥션 구조를 그대로 유지하는 클라우드 DB 확보 완료.

### Phase 2. 배포 파이프라인 대응을 위한 파일 수정
* **작업 내용**: 백엔드 및 프론트엔드가 클라우드 배포 환경에서 안정적으로 구동될 수 있도록 최소한의 보조 설정 파일 정비.
* **수정 및 생성된 파일 상세**:
  1. **`server/db.js` 수정 (SSL 및 외부 포트 허용)**:
     - 클라우드 DB 전용 포트와 SSL 암호화 통신을 처리할 수 있도록 `port: process.env.DB_PORT || 3306` 및 `ssl: process.env.DB_SSL === 'true' ? { rejectUnauthorized: false } : undefined` 옵션 추가.
     - 로컬 개발 환경(`localhost:3306`)에는 영향을 주지 않도록 환경 변수 기반 폴백(Fallback) 구조로 작성.
  ...
  3. **`.env.example` 템플릿 최신화**:
     - 공유용 템플릿에 추가 변수인 `DB_PORT=3306`, `DB_SSL=false` 키 명시. 실제 접속 정보가 담긴 `.env` 파일은 `.gitignore`로 안전하게 보호.
* **결과**: 기존 개발 환경을 해치지 않으면서 클라우드 배포 조건을 완벽히 수용.

### Phase 3. Vercel 모노레포 연동 및 대시보드 환경 변수(Environment Variables) 주입
* **작업 내용**: Vercel 호스팅 서비스를 GitHub 레포지토리와 연동하고 보안 환경 변수 주입.
* **진행 과정**:
  1. Vercel 프로젝트 생성 시 모노레포 구조에 맞춰 Root Directory를 **`client`**로 지정하고 Production Branch를 기본 브랜치로 세팅.
  2. Vercel 대시보드 `Environment Variables` 메뉴에 클라우드 DB 접속 정보(`DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASSWORD`, `DB_NAME`, `DB_SSL=true`) 등록.
...
* **진행 과정**:
  1. 작업 브랜치(`feature/deploy-setup`)에서 통합 브랜치(`develop`) 방향으로 PR(Pull Request) 오픈.
  2. Vercel Bot이 PR 댓글로 즉시 달아주는 **독립된 임시 Preview URL**에 접속하여 화면 UI 및 라이브 동작 사전 검수.
  3. 검수 완료 후, PR 승인 및 Merge 절차를 거쳐 통합 브랜치로 반영.
* **보안 및 외부 비공개 정책 (Security & Privacy)**:
  - **검색 엔진 차단**: Vercel이 `X-Robots-Tag: noindex`를 자동 설정하여 Preview 및 스테이징 사이트 노출 방지.
  - **주소 난수화**: 추측 불가능한 무작위 해시 URL로 생성되어 외부인의 무단 접근 차단.
  - **Private 저장소 보호**: Private 저장소 환경에서는 PR 댓글의 Preview 링크 조회를 허용된 구성원으로 제한.
...
* **진행 과정**:
  ...
  2. **환경 변수 및 신규 API 주입 확장성**: 향후 외부 API 키나 신규 기능 키가 필요할 경우, Vercel 대시보드에 환경 변수만 추가 등록해 두면 소스코드 및 클라우드 DB 연동이 자동으로 유지됨.
  3. **자동화된 무중단 릴리즈(CD)**: 통합 브랜치에 코드가 머지되는 순간 Vercel이 라이브 배포 사이트를 최신 상태로 자발적 갱신하여 릴리즈 공수를 제로화.
